...
logger = logging.getLogger("ctp_balance_tool")
...
async def _query_once(timeout_sec: float, max_fronts: int) -> dict:
    ...
    for md_front, td_front in fronts:
        ...
        try:
            ...
            logger.info("CTP connected, querying account...")
            account = await gateway.query_account()
            if account is None:
                raise RuntimeError("CTP query_account returned empty.")
...
async def _main() -> int:
    parser = argparse.ArgumentParser(description="Query CTP account balance.")
    parser.add_argument("--env-file", default=".env", help="Path to env file, e.g. .env1")
    parser.add_argument("--timeout-sec", type=float, default=35.0, help="Connect timeout per front")
    parser.add_argument("--hard-timeout-sec", type=float, default=45.0, help="Total timeout for whole query")
    parser.add_argument("--max-fronts", type=int, default=1, help="How many CTP fronts to try (default: primary only)")
    args = parser.parse_args()

    try:
        _load_env_file(args.env_file)
        result = await asyncio.wait_for(
            _query_once(args.timeout_sec, args.max_fronts),
